...
2) Start a session with `handoffkit session start` (or VS Code task).
...
5) At session end, write back to memory docs and commit with `handoffkit session end --commit`.
